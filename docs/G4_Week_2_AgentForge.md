...
A simple agent with reliable tool execution beats a complex agent that hallucinates or fails
unpredictably.
Choose Your Domain
...
Build domain-appropriate tools. Examples by domain (look through your chosen repo to identify
the best opportunities for tools):
Healthcare
...
Correctness Does the agent return accurate information? Fact-check against
ground truth.
...
Each test case must include: input query, expected tool calls, expected output, and pass/fail
criteria.
...
Trace Logging Full trace of each request: input → reasoning → tool calls →
output
